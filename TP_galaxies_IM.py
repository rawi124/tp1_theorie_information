...
def calcule_distribution_proba(liste_tirages):
    ...
    probas = []
    x = 0
    unique = np.unique(liste_tirages,axis=0)
    for uni in unique :
        x = 0
        for el in liste_tirages :
            if el == uni  :
                x = x + 1
        probas.append(x/len(liste_tirages))
...
def calcule_NMI(liste_tirages_varX, liste_tirages_varY):
    ...
    h_xy = []
    for x in liste_tirages_varX :
        for y in liste_tirages_varY :
            h_xy.append((x,y))
